...
Supporting services running alongside: **broker** (Confluent Kafka 7.6, KRaft mode — no ZooKeeper), **schema-registry** (`http://localhost:8081`), **kafka-ui** (`http://localhost:8080`).
...
This starts the broker, Schema Registry, Kafka UI, creates both topics, then launches the generator, detector, and reporter. Wait ~30 s for the broker healthcheck to pass before services begin producing/consuming.
